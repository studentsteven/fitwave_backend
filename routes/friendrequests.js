...
router.post('/friendrequests', (req, res) => {
    ...
    var checkUser = `SELECT u1.username AS username1, u2.username AS username2, f.sended, f.id FROM friend f JOIN user u1 ON f.user_id_1 = u1.id 
    JOIN user u2 ON f.user_id_2 = u2.id WHERE (f.user_id_1 = ? OR f.user_id_2 = ?) AND f.accepted = 0 AND NOT f.request_by = ?`;
    db.query(checkUser, [user_id, user_id, user_id], function (err, result) {
        if (err) throw err;
        res.send(result);
    });
});
